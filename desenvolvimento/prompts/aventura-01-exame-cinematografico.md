# Prompt personalizado — Aventura 01: Exame de Admissão

## Tarefa

Reestruturar `campanha/aventuras/01-exame-de-admissao.md` como uma aventura completa e jogável de abertura para **Herdeiros da Vanguarda**, combinando o rascunho vigente com o briefing explícito do usuário: largada diante de um bairro comercial simulado, caça a robôs pontuados, obstáculo estrutural, surgimento de um robô gigante que vale zero pontos, resgate de um candidato e resultado posterior.

## Fontes obrigatórias

- `campanha/01-premissa-e-tom.md`
- `campanha/02-estrutura-narrativa.md`
- `campanha/03-arcos.md`
- `campanha/arcos/arco-1/README.md`
- `campanha/diretrizes-de-design.md`
- `cenario/05-programa-de-campo.md`
- `cenario/11-instituto-atlas-hub-jogavel.md`
- `regras/05-operacoes-do-atlas.md`
- `regras/12-fichas-alunos-recorrentes.md`
- briefing do usuário nesta sessão

## Restrições de cânone

- Não acrescentar fatos ao segredo da campanha, ao Retorno ou aos Pacotes de Antagonista.
- Não transformar estudantes recorrentes aprovados em caricaturas incompatíveis com suas fichas.
- Usar Noah como prodígio estratégico e líder rival, sem torná-lo cruel ou incompatível com sua ficha aprovada.
- Preservar Tomás como observador e condutor de debriefing, sem revelar Multiplex.
- Todos os personagens jogadores ingressam no Atlas; falhas podem gerar plano de desenvolvimento, não expulsão da campanha.
- Não declarar como canônico o número de personagens jogadores. Balancear os encontros para 2–3 personagens.

## Objetivos de design

1. Criar uma aventura de uma sessão de aproximadamente 3–4 horas.
2. Abrir com impacto visual, discurso de recepção de Álvaro, passagem do comando pelo microfone para Tomás, aproximação pessoal de Samira para entrega dos Identificadores, rivalidade e contagem regressiva.
3. Dar a cada personagem uma oportunidade inicial de parecer competente.
4. Estruturar três encontros de combate consecutivos para uma equipe de 2–3 personagens de 10 pontos, usando fichas adaptadas de *3DeT Victory — Ferozes e Furiosos*: Robô de Combate Produzido em Massa (Modelo Antigo), P.E.A.O., G.A.R.I. e Robô de Duelo.
5. Aplicar as recomendações de Categoria de Dano, Grau de Ameaça, números fixos e recuperação entre cenas de combate.
6. Tratar o combate como conflito não violento: cada candidato recebe um Identificador de Avaliação Atlas, modular e acessível, que converte acertos simulados em perda de PV como margem operacional, nunca ferimento físico.
7. Tornar o prédio instável um objetivo simultâneo ao terceiro combate: contornar, abrir passagem ou estabilizar.
8. Fazer do Ponteiro Zero um Robô de Duelo Ningen, 17 pontos, forte mas possível de enfrentar, sem exigir sua derrota para concluir a prova.
9. No clímax, candidatos em 0PV precisam ser transportados à Área Azul. Cecília lidera a evacuação; Sofia é agarrada pelo Ponteiro Zero depois de salvar um candidato; Noah permanece para apoiar os personagens, que formam a última linha de defesa.
10. Tomás comanda a etapa prática pelo sistema de som, explica suas linhas gerais e comunica que, se o Ponteiro Zero atravessar o perímetro da Área Azul, todos falham na etapa coletiva.
11. Manter explícitos Controle, Julgamento, Equipe, Responsabilidade e Prioridade. A pontuação de combate não decide sozinha o ingresso.
12. Usar Meta 9 como padrão, Meta 12 para ações especialmente difíceis, além de Ganho, Perda, Ajuda e PA.
13. Dar informação indispensável sem exigir rolagem.
14. Organizar cada cena por propósito, situação, decisões, procedimento e consequências.
15. Encerrar com debriefing imediato e confirmação de ingresso por mensagem holográfica dias depois.

## Estrutura desejada

- visão geral operacional e pergunta dramática;
- preparação e elenco em foco;
- Cena 1: O Portão e a Largada;
- Cena 2: Circuito de Combate — Avenida das Vitrines, Galeria Vertical e Quarteirão em Obras;
- Cena 3: A Última Linha e o Ponteiro Zero;
- Cena 4: Debriefing e Resultado;
- quadro dos cinco critérios;
- fichas rápidas dos desafios;
- XP e registro de continuidade.

## Critérios de qualidade

- Linguagem direta para o Mestre.
- Situações abertas, não ação esperada obrigatória.
- Tom apropriado para crianças e pré-adolescentes.
- Segurança diegética do Ginásio Modular clara, sem neutralizar a urgência ficcional.
- Adultos apoiam e interrompem risco real, mas não resolvem a escolha central.
- Texto pronto para uso, sem depender de material externo além das regras básicas.
